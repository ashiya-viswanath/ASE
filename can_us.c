...
void CAN_Write(int messageBufferNo,msgPacket inputMsgPacket){
    ...
    /* Data Length */
    CAN_0.BUF[messageBufferNo].CS.B.LENGTH = (inputMsgPacket.dataLength<4 && inputMsgPacket.dataLength>=0)?inputMsgPacket.dataLength:4 ; //sending 4 bytes
    /* STD_ID */
    CAN_0.BUF[messageBufferNo].ID.B.STD_ID = inputMsgPacket.standardId ;
    for(int i=0; i<inputMsgPacket.dataLength;i++){
        //doesnt enter if dataLength is zero
        CAN_0.BUF[messageBufferNo].DATA.B[i] = inputMsgPacket.data & 0x000000ff;
